...
public static class MultiplayersBuildAndRun {
	...
	[MenuItem("File/Run Multiplayer/IOS/2 Players")]
	static void PerformOSXBuild2 (){
		PerformIOSXBuild (2);
	}

	[MenuItem("File/Run Multiplayer/IOS/3 Players")]
	static void PerformOSXBuild3 (){
		PerformIOSXBuild (3);
	}

	[MenuItem("File/Run Multiplayer/IOS/4 Players")]
	static void PerformOSXBuild4 (){
		PerformIOSXBuild (4);
	}

	static void PerformIOSXBuild (int playerCount)
	{
		EditorUserBuildSettings.SwitchActiveBuildTarget(BuildTargetGroup.Standalone,BuildTarget.iOS);
		for (int i = 1; i <= playerCount; i++) {
			BuildPipeline.BuildPlayer (GetScenePaths (), "Builds/IOSX/" + GetProjectName () + i.ToString() + ".app", BuildTarget.iOS, BuildOptions.AutoRunPlayer);
		}
